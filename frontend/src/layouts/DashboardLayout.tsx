...
import { Box, Button, Container, Drawer, Grid2, Typography, useColorScheme } from "@mui/material";
import { breadcrumbUrl, firstCharacterUppercase } from "../utils/Utils";
import { Outlet } from "react-router";
import { useAccount } from "@azure/msal-react";

function Branding({ title = "App", icon = undefined }) {
    return (
        <>
            <div className="DashboardBranding">
...
function Sidebar({ children }) {
    return (
        <>
            ...
            <Drawer variant="temporary" open={false} PaperProps={{
                sx: {
                    p: 2,
                    width: "auto"
                }
...
function ListSection({ sectionTitle = "Section", children }) {
    return (
        <>
            <Box className="DashboardListSection" sx={{
                my: {
                    xl: 3,
                    sm: 0,
                }
...
function ListItemLink({ title = "Link", pageTitle = "Page", link = "/", icon }) {
    return (
        <>
            <a className="DashboardListItemLink" href={link}>
                <span>{icon}</span>
                <Typography sx={{
                    display: {
                        xl: "block",
...
function ListItemButton({ title = "Button", handleClick, icon }) {
    return (
        <>
            <button type="button" className="DashboardListItemButton" onClick={handleClick}>
                <span>{icon}</span>
                <span>{title}</span>
            </button>
        </>
...
function PageHeader({ title = "Page" }) {
    ...
    return (
        <>
            <div className="DashboardPageHeader">
                ...
                <Button variant="text" startIcon={<IconUserCircle size={26} />} sx={{ color: "black" }}>
                    {userAccount?.name == undefined ? "Account" : userAccount.name}
                </Button>
            </div>
        </>
    )
...
function PageContent({ pageTitle, children }) {
    return (
        <>
...
function SectionDivider({ my = 10 }) {
    return (
        <>
            <hr style={{ marginTop: my, marginBottom: my }} />
        </>
    )
}
...
function PageBreadcrumbs() {
    ...
    const [hasCrumbs, setHasCrumbs] = useState<boolean>(false);

    useEffect(() => {
        // Get the breadcrumbs from the URL
        const breadcrumbs = breadcrumbUrl(window.location.pathname);

        // Check if this page has breadcrumbs first
        if (breadcrumbs.length > 1) {
            setHasCrumbs(true);

            // Variable for storing the final URL after crumbs
            var finalUrl: string = "";

            // Add suburls to crumbs state
            for (var i = 0; i < breadcrumbs.length; i++) {
                if (i == breadcrumbs.length - 1) {
                    finalUrl += firstCharacterUppercase(breadcrumbs[i]);
                } else {
                    finalUrl += firstCharacterUppercase(breadcrumbs[i]) + " > ";
                }
            }
    ...
    return (
        <>
            {hasCrumbs ? <span className="DashboardBreadcrumbs"><IconHome size={18} style={{ marginRight: 4 }} /> &gt; {crumbs}</span> : <></>}
        </>
    )
}
...
function Dashboard() {
    ...
    const handleThemeChange = () => {
        // This thing makes the screen flicker
        if (localStorage.getItem("mui-mode") === "dark") {
            setMode("light");
        } else {
            setMode("dark");
        }
    ...
    return (
        <>
            {/* <Grid templateColumns="repeat(12, 1fr)" style={{ height: "100vh" }}> */}
            <Grid2 container sx={{ height: "100vh" }}>
                <Grid2 size={2} sx={{ width: "auto" }}>
                    <Sidebar>
                        <Branding />
                        ...
                        <ListSection sectionTitle="Helpdesk">
                            <ListItemLink title="Tickets" link="/helpdesk/tickets" icon={<IconTicket />} />
                            <ListItemLink title="Knowledgebase" link="/helpdesk/kb" icon={<IconLibrary />} />
                            <ListItemLink title="Development" link="/helpdesk/development" icon={<IconCode />} />
                        </ListSection>
                        {/* <SectionDivider /> */}
                        ...
                        {/* <SectionDivider /> */}
                        <ListSection sectionTitle="Theme">
                            <ListItemButton title="Light or Dark" handleClick={handleThemeChange} icon={<IconPalette />} />
                        </ListSection>
                    </Sidebar>
                </Grid2>
                <Grid2 size={10} sx={{ flexGrow: 1 }}>
                    <Outlet />
                </Grid2>
            </Grid2>
            {/* </Grid> */}
